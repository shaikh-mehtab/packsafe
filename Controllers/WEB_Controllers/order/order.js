...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    if (payment_status !== "failed") {
      handleEmailSender(order_id).then(data=>{
        if (!data) {
          handleEmailSender(order_id)
        }
      })
    }
...
exports.getOrderDetails = async (req, res) => {
  try {
    ...
    const order_data = { ...data[0][0], prodcuts: ordered_products[0], store_setting: store_setting[0] };
...
exports.orderDetailsController = async (req, res) => {
  try {
    const id = req.params.id;
    const data = await db.query("SELECT orders.*,coupons.name as coupon_name,coupons.code as coupon_code, coupons.total_amount as discount_amount,discount as coupon_percen FROM `orders` LEFT join `coupons_used` on orders.id = coupons_used.order_id left join coupons on coupons_used.coupon_id = coupons.id WHERE orders.id=?", [id]);
    if (data[0][0].address_id) {
      const address = await db.query("select * from addresses where id =?", [
        data[0][0].address_id,
      ]);
...
function renderEjs(file, data, callback) {
  fs.readFile(file, 'utf8', (err, template) => {
    if (err) {
      callback(err);
      return;
    }
    const renderedFile = ejs.render(template, { order_data: data });
    callback(null, renderedFile);
  });
}


function sendInvoiceMail(s_email, c_email, pass,host,port, invoice, callback) {
  renderEjs(path.resolve(__dirname, "invoice.ejs"), invoice, (err, html) => {
    if (err) {
      console.error(err);
      callback(err);
      return;
    }
    console.log(s_email, pass);
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      callback(null, info); // Invoke the callback with null for error and info for success
    });
  });
}

const handleEmailSender = async (id) => {
  try {
    const data = await db.query("SELECT orders.*,coupons.name as coupon_name,coupons.code as coupon_code, coupons.total_amount as discount_amount,discount as coupon_percen FROM `orders` LEFT join `coupons_used` on orders.id = coupons_used.order_id left join coupons on coupons_used.coupon_id = coupons.id WHERE orders.id=?", [id]);
    if (data[0][0].address_id) {
      const address = await db.query("select * from addresses where id =?", [
    ...
    const [store_setting] = await db.execute(
      "select * from setting s join store_location sl "
    );

    const order_data = { ...data[0][0], products: ordered_products[0], store_setting: store_setting[0] };
    const mailer_email = store_setting[0].mailer_email
    const reciever_email = store_setting[0].mailer_notify_email +", "+order_data.email+", "+order_data.ship_email
    const mailer_password = store_setting[0].mailer_password
    const mailer_host = store_setting[0].mailer_host
    const mailer_port = Number(store_setting[0].mailer_port)
    sendInvoiceMail(store_setting[0].mailer_email,reciever_email,mailer_password,mailer_host,mailer_port,order_data,(err,info)=>{
      if (err) {
        throw new Error(err)
      }
    })
