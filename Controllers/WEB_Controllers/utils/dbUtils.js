...
exports.fetchProducts = async (page_no, categories, brands) => {
  try {
    ...
    if (categoryIds.length > 0) {
      whereClause.push(`p.category IN (?)`);
      mainParams.push(categoryIds);
    }

    if (brandIds.length > 0) {
      whereClause.push(`p.manufacturer IN (?)`);
      mainParams.push(brandIds);
    }
    ...
    mainParams.push(parseInt(limit, 10));
    mainParams.push(parseInt(offset, 10));
    ...
    if (categoryIds.length > 0) {
      countSql += ` AND p.category IN (?)`;
      countParams.push(categoryIds);
    }

    if (brandIds.length > 0) {
      countSql += ` AND p.manufacturer IN (?)`;
