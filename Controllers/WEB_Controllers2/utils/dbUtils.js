...
exports.fetchProducts = async (page_no, category) => {
  try {
    ...
    if (category) {
      ...
      if (cat[0].length > 0) {
        const totalProducts = await db.query(
          `select count(*) as total from products where category=? and status >0  `,
          [cat[0][0].id]
        );

        // const active_offers = await db.query("select * from splprices where status >=0 and from_date <=CURRENT_DATE and to_date >=CURRENT_DATE")

        // active_offers[0].forEach(offer=>{
        //  products[0].map //logiv
        // })

        // const products1 = await db.query(
        //   `select p.*,s.name as special_name,s.amount as discount_amount,(p.price- COALESCE(s.amount,0)) as spl_price from products p left join splprices s on s.p_id=p.id where p.category=? and p.status >0 order by p.sort_order limit ? offset ? `,
        //   [cat[0][0].id, limit, offset]
        // );
...
exports.fetchManf = async (slug) => {
  try {
    if (slug) {
      const manf = await db.query(
        "select manufacturer from categories where slug=? and status >=0",
        [slug]
      );
      if (manf[0].length > 0) {
        const list = await db.query(
          `select distinct m.name,m.slug from manufacturers m join categories c on FIND_IN_SET(m.id,?) > 0`,
          [manf[0][0].manufacturer]
        );
        return list[0];
      } else return [];
    } else {
...
exports.fetchAboutContent = async () => {
  try {
    const aboutContent = await db.execute(
      `SELECT about_title as page_title, about_meta_title as page_meta_title, about_meta_desc as page_meta_desc,about_cover as page_cover,about_breadcrumb_name as breadcrumb_name,about_text as content,about_group_data as group_data FROM web_contents ;`
    );
    const [store_setting] = await db.execute(
      "select s.logo,s.dark_logo,s.favicon,sl.* from setting s join store_location sl "
    );
